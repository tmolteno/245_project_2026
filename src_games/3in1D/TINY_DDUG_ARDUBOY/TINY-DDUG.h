...
#define GAME_FRAME_CONTROL_TDDUG while((currentMillis-MemMillis)<66){currentMillis=millis();}MemMillis=currentMillis
...
void SND_TDDUG(uint8_t Val_){
switch (Val_){
 case (0):for(uint8_t t_=1;t_<250;t_=t_+10){Sound(t_,4);Sound(0,15);Sound(255-t_,2);}break;
 case (1):for(uint8_t t_=1;t_<100;t_=t_+5){Sound(t_,4);}break;
 case (2):Sound(20,150);Sound(100,150);break;
default:break;  
   }
}
...
if (Universal_TDDUG(Dx_,Dy_,Dw_,Dh_,Enx_,Eny_,Enh_,Enw_)) {for (uint8_t t4=200;t4>2;t4-=4){Sound(t4,1);}RESET_WEAPON_TDDUG();GD_DDUG.DEAD++;}
}}}
...
return ENEMY_DDUG[t_].DIRECTION_X();}else{
  if (RecupeDecalageY(ENEMY_DDUG[t_].X()-20)==0) {
...
return ENEMY_DDUG[t_].DIRECTION_Y();}else{
    if (RecupeDecalageY(ENEMY_DDUG[t_].Y())==0) {
...
if (Y_>11) return 1;
if (Y_<0) return 0;
if (((0b10000000>>RecupeDecalageY(X_))&GRID_TDDUG[Y_][X_>>3])!=0) {return 1;} else{return 0;}
}
...
GRID_TDDUG[Y_][X_>>3]=GRID_TDDUG[Y_][X_>>3]&(0xff-(0b10000000>>RecupeDecalageY(X_)));
return 0;
}
...
uint8_t MAIN_TDDUG(uint8_t xPASS,uint8_t yPASS){
return blitzSprite(DUG_TDDUG.X(),DUG_TDDUG.Y(),xPASS,yPASS,GD_DDUG.MAIN_FRAME,Dig_TDDUG);
}

uint8_t RECUPE_ENEMY_TDDUG(uint8_t xPASS,uint8_t yPASS){
uint8_t Byte_=0x00,t_;
for (t_=0;t_<MAX_ENEMY_TDDUG;t_++){
if (ENEMY_DDUG[t_].ACTIVE()!=0) {Byte_=Byte_|blitzSprite(ENEMY_DDUG[t_].X(),ENEMY_DDUG[t_].Y(),xPASS,yPASS,ENEMY_DDUG[t_].ANIM(),Sprite_ENEMY_TDDUG);}
}
return (Byte_);
}
...
if ((W_TDDUG[0].ACTIVE()!=0)&&(W_TDDUG[0].ACTIVE()!=5)) {
  RESULT=blitzSprite(W_TDDUG[0].X(),W_TDDUG[0].Y(),xPASS,yPASS,W_TDDUG[0].ANIM_OR(),LAZER_TDDUG);
  if (W_TDDUG[1].ACTIVE()!=0) {RESULT|=blitzSprite(W_TDDUG[1].X(),W_TDDUG[1].Y(),xPASS,yPASS,W_TDDUG[0].ANIM_OR(),LAZER_TDDUG);}
 }else{
    RESULT=0;
}
return RESULT;
...
switch(Select_){
 case 0:Byte1_=recupe_SCORES_TDDUG(x,y,-33)|SPEED_BLITZ(46,2,x,y,0,DDUG_INTRO_TDDUG)|RECUPE_START_TDDUG(x,y,Blink_);break;
 case 1:Byte1_=recupe_SCORES_TDDUG(x,y,0)|Recupe_TDDUG(x,y);break;
 default:break;
}
arduboy.SPItransfer(Byte1_);
eend:;
...
return SPEED_BLITZ(51,6,xPASS,yPASS,0,START_TDDUG);
}
...
if (yPASS<7) {return 0;}
return 
(SPEED_BLITZ(85+Pos_,7,xPASS,yPASS,M10000_TDDUG,police_TDDUG)|
 SPEED_BLITZ(89+Pos_,7,xPASS,yPASS,M1000_TDDUG,police_TDDUG)|
 SPEED_BLITZ(93+Pos_,7,xPASS,yPASS,M100_TDDUG,police_TDDUG)|
 SPEED_BLITZ(97+Pos_,7,xPASS,yPASS,M10_TDDUG,police_TDDUG)|
 SPEED_BLITZ(101+Pos_,7,xPASS,yPASS,M1_TDDUG,police_TDDUG)|
 SPEED_BLITZ(105+Pos_,7,xPASS,yPASS,0,police_TDDUG));
}
