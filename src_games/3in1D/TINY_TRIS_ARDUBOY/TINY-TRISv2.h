...
//for delete HIGHSCORE, at the start menu, press right and left in same time for 2 sec.
#include<EEPROM.h> //ADRESS USE 0x05-0x06-0x07-0x08-0x09-0x0A
...
void RESET_HIGHSCORE_TTRIS(void);
...
if (DROP_BREAK_TTRIS==6) {
  END_DROP_TTRIS();
  if (End_Play_TTRIS()) {  Tiny_Flip_TTRIS(128);SND_TTRIS(3); delay(2000);Check_NEW_RECORD();goto MENU;}
...
if (BUTTON_DOWN) {reset_Score_TTRIS();break;}
  ...
  if ((TINYJOYPAD_RIGHT==1)&&(TINYJOYPAD_LEFT==1)) {
for (Timer1=0;Timer1<60;Timer1++){
if (!((TINYJOYPAD_RIGHT==1)&&(TINYJOYPAD_LEFT==1))) {goto BYPASS;}
  delay(33);
  Sound(140,4);
  Timer1++;
}
RESET_HIGHSCORE_TTRIS();
while(1){
  if (!((TINYJOYPAD_RIGHT==1)&&(TINYJOYPAD_LEFT==1))) {goto RELOAD;break;}
  }
 }
BYPASS:;
delay(33);
...
if ((TINYJOYPAD_RIGHT==0)&&(TINYJOYPAD_LEFT==0)) {LONG_PRESS_X_TTRIS=0;PSEUDO_RND_TTRIS();}
if (BUTTON_UP) {
...
uint8_t Y_VAR_DECALAGE=RecupeDecalageY(Y_SCAN);
...
if (GRID_STAT_TTRIS(x,y)==1) {BYTE_TTRIS=BYTE_TTRIS|blitzSprite(46+(x*3),5+(y*3),xPASS,yPASS,0,tinyblock_TTTRIS);}
}
...
for (uint8_t y=0;y<5;y++){
for (uint8_t x=0;x<5;x++){
if (Scan_Piece_Matrix_TTRIS(x,y+(PIECEs_TTRIS_PREVIEW*5))==1) {Byte_Mem|=blitzSprite(92+(x*2)+x_add,(27+(y*2))-5+y_add,xPASS,yPASS,0,tiny_PREVIEW_block_TTTRIS);}
}}
return Byte_Mem;
}
return 0;
...
for (uint8_t y=0;y<5;y++){
for (uint8_t x=0;x<5;x++){
if (Piece_Mat2_TTRIS[x][y]==1) {Byte_Mem|=blitzSprite(xx_TTRIS+(x*3),(yy_TTRIS+(y*3))-5,xPASS,yPASS,0,tinyblock2_TTTRIS);}
}}
return Byte_Mem;
}
...
for (x = 0; x < 128; x++){
  if (x<83) {
    arduboy.SPItransfer(Recupe_TTRIS(x,y));
    }else{
      arduboy.SPItransfer(TAB[x-83][y]);
      }
  }
}}
...
uint8_t y,x; 
for (y = 0; y < 8; y++){ 
for (x = 0; x < 128; x++){arduboy.SPItransfer(intro_TTRIS(x,y,TIMER1));}
}}
...
uint8_t Recupe_Start_TTRIS(uint8_t xPASS,uint8_t yPASS,uint8_t *TIMER1){
if (*TIMER1>7) {
  return blitzSprite(49,28,xPASS,yPASS,0,start_button_1_TTRIS)|blitzSprite(49,36,xPASS,yPASS,0,start_button_2_TTRIS);
  }else{
    return 0;
    }
}
...
return 
(blitzSprite(95,8,xPASS,yPASS,M10000,police_TTRIS)|
 blitzSprite(99,8,xPASS,yPASS,M1000,police_TTRIS)|
 blitzSprite(103,8,xPASS,yPASS,M100,police_TTRIS)|
 blitzSprite(107,8,xPASS,yPASS,M10,police_TTRIS)|
 blitzSprite(111,8,xPASS,yPASS,M1,police_TTRIS)|
 blitzSprite(115,8,xPASS,yPASS,0,police_TTRIS));
}
...
if (yPASS>1) {return 0;}
return 
(blitzSprite(16,8,xPASS,yPASS,Nb_of_line_TTRIS[2],police_TTRIS)|
 blitzSprite(20,8,xPASS,yPASS,Nb_of_line_TTRIS[1],police_TTRIS)|
 blitzSprite(24,8,xPASS,yPASS,Nb_of_line_TTRIS[0],police_TTRIS));
}
...
if (yPASS!=5)  {return 0;}
return 
(blitzSprite(109,41,xPASS,yPASS,(Level_TTRIS/10),police_TTRIS)|
 blitzSprite(114,41,xPASS,yPASS,(Level_TTRIS%10),police_TTRIS));
}
...
void recupe_HIGHSCORE_TTRIS(void){
if (EEPROM.read(EEPROM_STORAGE_SPACE_START+6)!=0b10101010){goto ED;}
Level_TTRIS=EEPROM.read(EEPROM_STORAGE_SPACE_START+1);
Nb_of_line_F_TTRIS=(EEPROM.read(EEPROM_STORAGE_SPACE_START+2)<<8);
Nb_of_line_F_TTRIS=Nb_of_line_F_TTRIS|(EEPROM.read(EEPROM_STORAGE_SPACE_START+3));
Scores_TTRIS=(EEPROM.read(EEPROM_STORAGE_SPACE_START+4)<<8);
Scores_TTRIS=Scores_TTRIS|(EEPROM.read(EEPROM_STORAGE_SPACE_START+5));
ED:;
}

void RESET_HIGHSCORE_TTRIS(void){
EEPROM.write(EEPROM_STORAGE_SPACE_START+1,0);
EEPROM.write(EEPROM_STORAGE_SPACE_START+2,0);
EEPROM.write(EEPROM_STORAGE_SPACE_START+3,0);
EEPROM.write(EEPROM_STORAGE_SPACE_START+4,0);
EEPROM.write(EEPROM_STORAGE_SPACE_START+5,0);
EEPROM.write(EEPROM_STORAGE_SPACE_START+6,0b10101010);
}

void save_HIGHSCORE_TTRIS(void){
uint8_t TMP;
EEPROM.write(EEPROM_STORAGE_SPACE_START+1,Level_TTRIS);
TMP=Nb_of_line_F_TTRIS>>8;
EEPROM.write(EEPROM_STORAGE_SPACE_START+2,TMP);
TMP=(Nb_of_line_F_TTRIS<<8)>>8;
EEPROM.write(EEPROM_STORAGE_SPACE_START+3,TMP);
TMP=Scores_TTRIS>>8;
EEPROM.write(EEPROM_STORAGE_SPACE_START+4,TMP);
TMP=(Scores_TTRIS<<8)>>8;
EEPROM.write(EEPROM_STORAGE_SPACE_START+5,TMP);
EEPROM.write(EEPROM_STORAGE_SPACE_START+6,0b10101010);
}

void Check_NEW_RECORD(void){
if ((Scores_TTRIS>uint16_t((EEPROM.read(EEPROM_STORAGE_SPACE_START+4)<<8)|(EEPROM.read(EEPROM_STORAGE_SPACE_START+5))))||(EEPROM.read(EEPROM_STORAGE_SPACE_START+6)!=0b10101010)) {
