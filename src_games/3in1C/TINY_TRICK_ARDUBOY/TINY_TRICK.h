...
uint8_t GOAL_TTRICK(uint8_t Goal_0,uint8_t Goal_1);
...
void Drag_Puck_test_TTRICK(void);
...
void intro_TTRICK(uint8_t GOAL_SCREEN,uint8_t SLIDE);
...
default:break;}
if (BUTTON_DOWN) {SND_TTRICK(7);_delay_ms(100);goto Start_Game;}
}}
Start_Game:;
RESTART:;
...
COM_MAX_SPEED_x=28;
COM_MAX_SPEED_y=28;
if  (TINYJOYPAD_UP) {Goal_Adj_TTRICK(0,3);ORIx=2;Speed_adj_V_N_TTRICK(0);}
else if (TINYJOYPAD_DOWN) {Goal_Adj_TTRICK(1,3);ORIx=8;Speed_adj_V_P_TTRICK(0);}else{ORIx=0;Speed_V_Decel_TTRICK(0);}
if (TINYJOYPAD_RIGHT) {ORIy=4; Speed_adj_H_P_TTRICK(0);}
else if (TINYJOYPAD_LEFT) {ORIy=16;Speed_adj_H_N_TTRICK(0);}else{ORIy=0;Speed_H_Decel_TTRICK(0);}
Adjust_Cumule_Speed_TTRICK(0);
Player1_Rotate=Direction_Recupe_TTRICK(ORIy+ORIx);
if (BUTTON_DOWN) {if ((Drag_Puck==0)&&(sprite_TTRICK[2].Xpos<119)) {KICK_PUCK_TTRICK(Player1_Rotate);}}
collision_check_LimiteScreen_TTRICK(0);
COM_MAX_SPEED_x=Adjust_Speed_Computer_TTRICK(X_Drag,sprite_TTRICK[1].Xpos);
COM_MAX_SPEED_y=Adjust_Speed_Computer_TTRICK(Y_drag,sprite_TTRICK[1].Ypos);
Computer_Rotate=Update_Computer_Direction_TTRICK(); 
...
Drag_Puck_test_TTRICK();
Drag_Puck_Update_TTRICK();

while((currentMillis-MemMillis)<50){
...
collision_check_LimiteScreen_TTRICK(2);
  if ((Drag_Puck==1)&&(sprite_TTRICK[1].Xpos<48)&&((Computer_Rotate>=3)&&(Computer_Rotate<=5))) {if ((sprite_TTRICK[2].Xpos>8)) {KICK_PUCK_TTRICK(Computer_Rotate);}}
  if (GOAL_!=255) {
   GOAL_=255;
   SND_TTRICK(1); 
   Drag_Puck=255;
   if (Points_CPU==10) {intro_TTRICK(3,0);SND_TTRICK(6);_delay_ms(1000);_delay_ms(1000);goto INTRO;}
   else if (Points_Pl1==10) {intro_TTRICK(2,0);SND_TTRICK(5);_delay_ms(1000);_delay_ms(1000);goto INTRO;}
...
if (Drag_Puck==255){
if (Xt1<64) {
if ((Xt1+2)<=(sprite_TTRICK[3].Xpos+1)&&(Goal_0==0)) {GOAL_=1;Points_CPU++;} }else{
if (((Xt1)>=((sprite_TTRICK[4].Xpos+5)-1))&&(Goal_1==0)) {GOAL_=0;Points_Pl1++;} }
}
return 1;
}
...
if ((YY>=23)&&(YY<=38)) {if (GOAL_TTRICK(Gar0,Gar1)) {return 0;}}
}
...
Puck_x=sprite_TTRICK[Drag_Puck].Xpos;
Puck_y=sprite_TTRICK[Drag_Puck].Ypos; 
sprite_TTRICK[2].Xpos=Puck_x+pgm_read_byte(&PLAYER_Puck_TTRICK[(Rot*2)]);
sprite_TTRICK[2].Ypos=Puck_y+pgm_read_byte(&PLAYER_Puck_TTRICK[(Rot*2)+1]);
}else{
Puck_x=sprite_TTRICK[2].Xpos;
Puck_y=sprite_TTRICK[2].Ypos;
}}
...
X_Drag=Puck_x;
Y_drag=Puck_y;}
if ((sprite_TTRICK[1].Xpos+W_PL2)<X_Drag) {Speed_adj_H_P_TTRICK(1);}else{Speed_adj_H_N_TTRICK(1);}
if ((sprite_TTRICK[1].Ypos+W_PL2)<Y_drag) {Speed_adj_V_P_TTRICK(1);}else{Speed_adj_V_N_TTRICK(1);}
...
void Speed_adj_H_P_TTRICK(uint8_t SPRITE_NO){
if (sprite_TTRICK[SPRITE_NO].SpeedXnegatif==0) {if (sprite_TTRICK[SPRITE_NO].SpeedXpositif<COM_MAX_SPEED_x) {sprite_TTRICK[SPRITE_NO].SpeedXpositif=sprite_TTRICK[SPRITE_NO].SpeedXpositif+2;}}else{
sprite_TTRICK[SPRITE_NO].SpeedXnegatif=sprite_TTRICK[SPRITE_NO].SpeedXnegatif-2;  
}}

void Speed_adj_H_N_TTRICK(uint8_t SPRITE_NO){
if (sprite_TTRICK[SPRITE_NO].SpeedXpositif==0) {if (sprite_TTRICK[SPRITE_NO].SpeedXnegatif<COM_MAX_SPEED_x) {sprite_TTRICK[SPRITE_NO].SpeedXnegatif=sprite_TTRICK[SPRITE_NO].SpeedXnegatif+2;}}else{
sprite_TTRICK[SPRITE_NO].SpeedXpositif=sprite_TTRICK[SPRITE_NO].SpeedXpositif-2;  
}}

void Speed_adj_V_P_TTRICK(uint8_t SPRITE_NO){
if (sprite_TTRICK[SPRITE_NO].SpeedYnegatif==0) {if (sprite_TTRICK[SPRITE_NO].SpeedYpositif<COM_MAX_SPEED_y) {sprite_TTRICK[SPRITE_NO].SpeedYpositif=sprite_TTRICK[SPRITE_NO].SpeedYpositif+2;}}else{
sprite_TTRICK[SPRITE_NO].SpeedYnegatif=sprite_TTRICK[SPRITE_NO].SpeedYnegatif-2;  
}}

void Speed_adj_V_N_TTRICK(uint8_t SPRITE_NO){
if (sprite_TTRICK[SPRITE_NO].SpeedYpositif==0) {if (sprite_TTRICK[SPRITE_NO].SpeedYnegatif<COM_MAX_SPEED_y) {sprite_TTRICK[SPRITE_NO].SpeedYnegatif=sprite_TTRICK[SPRITE_NO].SpeedYnegatif+2;}}else{
...
return (Patinoire1_2_TTRICK(xPASS,yPASS))
  &((0xff-blitzSprite(XP1,YP1,xPASS,yPASS,Player1_Rotate,PLAYER_WHITE_B_TTRICK)))
  &((0xff-blitzSprite(XP4,YP4,xPASS,yPASS,0,GOALER_WHITE_TTRICK)))
  &((0xff-blitzSprite(XP3,YP3,xPASS,yPASS,0,puck_TTRICK)))
  &((0xff-blitzSprite(XP5,YP5,xPASS,yPASS,0,GOALER_BLACK_TTRICK)))
  &(0xff-blitzSprite(XP2,YP2,xPASS,yPASS,Computer_Rotate,PLAYER_BLACK_B_TTRICK));
}
...
uint8_t y,x; 
for (y = 0; y < 8; y++){ 
for (x = 0; x < 128; x++){arduboy.SPItransfer(Recupe_TTRICK(x,y));}
}}
...
for (x = 0; x < 128; x++){
switch(GOAL_SCREEN){
  case 0:arduboy.SPItransfer((pgm_read_byte(&Intro_TTRICK[(x)+(y*128)])|blitzSprite(40,39,x,y,0,cadre40_37_TTRICK)));break;
  case 1:arduboy.SPItransfer(((Recupe_TTRICK(x,y)|(blitzSprite(40,39+SLIDE,x,y,0,screen_TTRICK)))&(0xff-blitzSprite(40,39+SLIDE,x,y,1,screen_TTRICK)))|(blitzSprite(40+(13),39+SLIDE+(13),x,y,Points_Pl1,NUM_TTRICK)|blitzSprite(40+(33),39+SLIDE+(13),x,y,Points_CPU,NUM_TTRICK)));break;
  case 2:arduboy.SPItransfer(Lost_win_TTRICK(x,y,win_TTRICK));break;
  case 3:arduboy.SPItransfer(Lost_win_TTRICK(x,y,lost_TTRICK));break;
  case 4:arduboy.SPItransfer(pgm_read_byte(&Intro_TTRICK[(x)+(y*128)]));break;
  default:break;
}}
...
uint8_t Lost_win_TTRICK(uint8_t xPASS,uint8_t yPASS,const uint8_t *SPRITES ){
return (((Recupe_TTRICK(xPASS,yPASS)|(blitzSprite(48,27,xPASS,yPASS,0,SPRITES))))&(0xff-blitzSprite(48,27,xPASS,yPASS,1,SPRITES)));
}
