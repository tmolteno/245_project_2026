...
arduboy.SPItransfer(RECUPE_TPIPE(x2,y));
}
}}
...
for (y = 0; y < 8; y++){ 
for (x = 0; x < 128; x++){ 
arduboy.SPItransfer(((FLIP_MODE_==1)?SPEED_BLITZ(38,5,x,y,0,START_TPIPE):0)|(pgm_read_byte(&TITLE_TPIPE[x+((y)*128)])));
}
}}
...
for (y = 0; y < 8; y++){ 
for (x = 0; x < 128; x++){ 
arduboy.SPItransfer(SPEED_BLITZ(44,3,x,y,0,LEVEL_TPIPE)|SPEED_BLITZ(75,3,x,y,GP.DIGIT2,police_TPIPE)|SPEED_BLITZ(80,3,x,y,GP.DIGIT1,police_TPIPE));
}
}
