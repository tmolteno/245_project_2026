...
#undef DEBUG // MC: This is defined somewhere in my libraries, and enables cheat mode
//#define DEBUG
//#define DEVKIT
...
void propagate(byte x, byte y) {
    ...
    if (state != STATE_LOSE && arduboy.audio.enabled()) tunes.tone(587, 20);
...
void settings() {
    ...
    if (getButtonDown(A_BUTTON) || getButtonDown(B_BUTTON)) {
        if (menuPosition == 0) {
            ...
            fastMode = !fastMode;
            if (arduboy.audio.enabled()) tunes.tone(587, 40);
        }
...
void playGame() {
    ...
    if (getButtonDown(A_BUTTON)) {
        ...
        if (isFlagged(selectedX, selectedY)) {
            if (arduboy.audio.enabled()) tunes.tone(800, 50);
            unsetFlag(selectedX, selectedY);
        } else if (!isOpen(selectedX, selectedY)) {
            if (arduboy.audio.enabled()) tunes.tone(980, 50);
...
void enterInitials() {
    ...
    while (true) {
        ...
        if (getButtonDown(LEFT_BUTTON) || getButtonDown(B_BUTTON)) {
            ...
            if (arduboy.audio.enabled()) tunes.tone(1046, 50);
        }
        ...
        if (clickButton(DOWN_BUTTON)) {
            initials[index]++;
            if (arduboy.audio.enabled()) tunes.tone(523, 25);
            // A-Z 0-9 :-? !-/ ' '
            if (initials[index] == '0') {
                initials[index] = ' ';
